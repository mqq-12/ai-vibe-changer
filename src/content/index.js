(() => {
  ...
  function adaptPage(reason = "manual") {
    ...
    try {
      ...
      const sidebar = applySidebarMarker(cachedElement("sidebar", findSidebar));
      const composer = mark(cachedElement("composer", findComposer), "composer");
      const main = mark(cachedElement("main", () => findMain(composer, sidebar)), "main");
      const header = mark(cachedElement("header", () => findHeader(main, sidebar)), "header");
      markComposerControls(composer);
      markComposerDock(composer);
      const codeBlockCount = markCodeBlocks(main);
      const thinkingCount = markThinking(main);
      const messageResult = markMessages(main, composer);
      const messageCount = messageResult.user + messageResult.assistant;
      const dialogCount = markDialogs();
  ...
  function cleanup() {
    ...
    for (const key of [
      ...
      "dbSkinHeader", "dbSkinSidebar", "dbSkinComposer", "dbSkinMessages", "dbSkinThinking", "dbSkinDialogs"
